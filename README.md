...
```mermaid
graph LR
    Input --> Pre[Deterministic Pre-Process]
    Pre --> Context[Curated Context Window]
    Context --> Sandbox[V8/Firecracker Isolate]
    Sandbox --> LLM[Commodity Model]
    LLM --> Schema[JSON Schema Validator]
    Schema --> Gate[HUMAN VERIFICATION GATE]
    style Sandbox fill:#444,color:#fff
    style Gate fill:#ff4,color:#000,stroke-width:4px
```
